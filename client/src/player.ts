...
class Player {
    ...
    actx: any = Tone.context;

    dest: any = this.actx.createMediaStreamDestination();

    recorder: any = new MediaRecorder(this.dest.stream);
    ...
    async load() {
        ...
        for (const sampleLoop of this.currentTrack.sampleLoops) {
            const samplePlayer = this.samplePlayers.get(sampleLoop.sampleGroupName)[sampleLoop.sampleIndex];
        ...
        for (const noteTiming of this.currentTrack.instrumentNotes) {
            ...
            instrumentSampler.connect(this.dest);
        ...
        Tone.Transport.scheduleRepeat((time) => {
            ...
            this.recorder.ondataavailable = (evt: any) => {
                console.log('recorder.ondataavailable', evt.data);
                chunks.push(evt.data);
            };

            this.recorder.onstop = (evt: any) => {
                const blob = new Blob(chunks, { type: 'audio/webm; codecs=opus' });
                const url = URL.createObjectURL(blob);
                const anchor = document.createElement('a');
                anchor.download = `${this.currentTrack.outputParams.title}.webm`;
                anchor.href = url;
                anchor.click();
            };
    ...
    playNext() {
        ...
        if (this.shuffle) {
            ...
            nextTrackIndex = this.currentPlayingIndex + 1;
        } else if (this.currentPlayingIndex === this.playlist.length - 1 && this.repeat === RepeatMode.ALL) {
            nextTrackIndex = 0;
        }
